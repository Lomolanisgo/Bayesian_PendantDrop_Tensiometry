...
# gen_single_drop.m:106
        A32=diag(ones(N,1))
# gen_single_drop.m:107
        A33=dot(dot(C,sigmaprime),D) + dot(sigmaprime,diag(cos(psi) / r))
# gen_single_drop.m:108
        A38=dot(sigmaprime,(dot(D,psi)))
# gen_single_drop.m:109
        A39=- ones(N,1)
# gen_single_drop.m:110
        b3=p0 - z - dot(sigmaprime,(dot(dot(C,D),psi) + sin(psi) / r))
# gen_single_drop.m:111
        # NOTE: the lengths are scaled with the radius, thus its value is one
        A81=fliplr(IDL)
# gen_single_drop.m:115
        b8=(1 - r(end()))
# gen_single_drop.m:115
        A91=multiply(multiply(dot(2,w),r.T),sin(psi.T))
# gen_single_drop.m:118
        A93=dot(multiply(w,r.T ** 2.0),cos(psi.T))
# gen_single_drop.m:119
        A98=- volume0prime / pi
# gen_single_drop.m:120
        b9=- (dot(w,(dot(r ** 2.0,sin(psi)))) - dot(C,volume0prime) / pi)
# gen_single_drop.m:121
        A11[1,arange()]=IDL
# gen_single_drop.m:124
        A13[1,arange()]=ZL
# gen_single_drop.m:125
        A18[1]=0
# gen_single_drop.m:126
        b1[1]=- r(1)
# gen_single_drop.m:127
        A22[1,arange()]=fliplr(IDL)
# gen_single_drop.m:130
        A23[1,arange()]=ZL
# gen_single_drop.m:131
        A28[1]=0
# gen_single_drop.m:132
        b2[1]=- z(end())
# gen_single_drop.m:133
        A31[1,arange()]=ZL
# gen_single_drop.m:136
        A32[1,arange()]=ZL
# gen_single_drop.m:137
        A33[1,arange()]=IDL
# gen_single_drop.m:138
        A38[1,arange()]=0
# gen_single_drop.m:139
        A39[1,arange()]=0
# gen_single_drop.m:140
        b3[1]=- psi(1)
# gen_single_drop.m:141
        Z1=zeros(N,1)
# gen_single_drop.m:144
...
# gen_single_drop.m:146
        b=concat([[b1],[b2],[b3],[b8],[b9]])
# gen_single_drop.m:150
        u=numpy.linalg.solve(A,b)
# gen_single_drop.m:153
        r=r + dot(alpha,u(arange(1,N)))
# gen_single_drop.m:156
        z=z + dot(alpha,u(arange(N + 1,dot(2,N))))
# gen_single_drop.m:157
        psi=psi + dot(alpha,u(arange(dot(2,N) + 1,dot(3,N))))
# gen_single_drop.m:158
        C=C + dot(alpha,u(dot(3,N) + 1))
# gen_single_drop.m:159
        p0=p0 + dot(alpha,u(dot(3,N) + 2))
# gen_single_drop.m:160
        if rms(b) > 1000.0:
            crash=1
# gen_single_drop.m:163
            break
